...
for change_index in range(len(changes_json)):
    ...
    if pre_final[0] is not None:
        ...
        print('Pre date boundary image with id chosen for change polygon ', change_index)
    else:
    ...
    if post_final[0] is not None:
        ...
        print('Post date boundary image with id chosen for change polygon ', change_index)
    else:
        null_data = {'change_poly_index': [change_index], 'catalog_id': [None], 'acq_date':
            [None], 'off_nadir': [None], 'multi_res': [None], 'pan_res': [None]}
...
with open('/home/output/pre_date_images.json', 'w') as f:
    f.write(pre_images.to_json(orient='records', lines=True))

with open('/home/output/post_date_images.json', 'w') as f:
    f.write(post_images.to_json(orient='records', lines=True))
print('save complete.  See your volume mount for output JSONs.')
